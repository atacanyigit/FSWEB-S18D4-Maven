...
class MainTest {
    ...
    void testFindById_Exists() {
        Burger burger = new Burger();
        burger.setId(1);
        when(entityManager.find(Burger.class, 1)).thenReturn(burger);
        Burger found = burgerDao.findById(1);
    ...
    void testFindById_NotExists() {
        ...
        assertThrows(BurgerException.class, () -> burgerDao.findById(999));
    }
    ...
    void testRemove() {
        ...
        // EntityManager'ın 1 numaralı Burger'i bulmasını sağla (1L değil 1)
        when(entityManager.find(Burger.class, 1)).thenReturn(burger);

        doNothing().when(entityManager).remove(burger);

        // remove(1) metodunu çağırıyoruz
        Burger removed = burgerDao.remove(1);

        assertNotNull(removed); // Nesne null değil mi?
